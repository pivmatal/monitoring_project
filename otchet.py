...
filename = f'/home/ufk/monitoring_project/static/otchet_pif_{date_str}.xlsx'
shutil.copy('/home/ufk/monitoring_project/static/templates/report_pif.xlsx', filename)
...
for uk in uks:
    ...
    if uk.uk_extractor:
        try:
            ...
            module = None
            print(f'Модуль {uk.uk_extractor} не загружен: {e}')
    ...
    for pif in PIF.objects.filter(Q(pif_uk = uk)):
        ...
        if pif.pif_checkpage and module:
            try:
                ext = module.Extractor(pif.pif_checkpage)
                ext.scrape()
                content = filter_out(ext.get_data(21), '.*справ.*стоим.*чис.*актив.*;;;.*справ.*СЧА.*;;;.*СЧА.*;;;.*справ.*стоим.*актив.*')
                if len(content) == 0:
                    content = [['Справка СЧА', localtime.localize(datetime.strptime('01.01.1970 00:00', '%d.%m.%Y %H:%M')), '']]
                content += filter_out(ext.get_data(21), '.*прирос.*стоим.*имущ.*;;;.*умень.*стоим.*имущ;;;.*отчет.*прирос.*;;;.*отчёт.*прирос.*')
                if len(content) < 2:
                    content += [['Отчет о приросте имущества', localtime.localize(datetime.strptime('01.01.1970 00:00', '%d.%m.%Y %H:%M')), '']]
